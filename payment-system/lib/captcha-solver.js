...
class TwoCaptchaSolver {
    ...
    try {
      ...
      if (proxy) {
        ...
        if (proxy.username && proxy.password) {
          submitData.proxy = proxy.username + ':' + proxy.password + '@' + submitData.proxy;
        }
      }
  ...
  async solveRecaptchaV3(sitekey, pageUrl, action = 'verify', minScore = 0.3, proxy = null) {
    try {
      ...
      const captchaId = submitResponse.data.request;
      console.log(`✅ reCAPTCHA v3 submitted to 2captcha. Task ID: ${captchaId}`);
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt++) {
      try {
        ...
        if (resultResponse.data.status === 1) {
          console.log(`✅ CAPTCHA solved successfully after ${attempt} attempts`);
          return resultResponse.data.request;
        } else if (resultResponse.data.error_text === 'CAPCHA_NOT_READY') {
          console.log(`⏳ CAPTCHA not ready yet (attempt ${attempt}/${this.maxAttempts})`);
          continue;
        } else {
          throw new Error(`2captcha error: ${resultResponse.data.error_text || 'Unknown error'}`);
        }
      } catch (error) {
        if (error.message.includes('CAPCHA_NOT_READY') || attempt < this.maxAttempts) {
          console.log(`⏳ Waiting for captcha result... (attempt ${attempt}/${this.maxAttempts})`);
          continue;
        }
        throw error;
      }
  ...
  async getBalance() {
    try {
      ...
      if (response.data.startsWith('ERROR_')) {
