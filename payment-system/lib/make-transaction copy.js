...
async function main() {
  ...
  try {
    ...
    const amountInputHandle = await page.evaluateHandle(() => {
      const divs = Array.from(document.querySelectorAll("div"));
      const targetDiv = divs.find((div) =>
        div.textContent.includes("You send"),
      );
      return targetDiv ? targetDiv.querySelector('input[type="text"]') : null;
    });
    if (amountInputHandle.asElement()) {
      const amountInput = amountInputHandle.asElement();
      await amountInput.evaluate((input) => (input.value = ""));
